...
function findByFilter() {
  ...
  $("input[id$='Filter']").each(function (index, item) {
    if (
      item.id != "partitionKeyFilter" &&
      item.id != "rowKeyDateStartFilter" &&
      item.id != "rowKeyDateEndFilter" &&
      item.id != "rowKeyTimeStartFilter" &&
      ...
      if ($(itemId).val() != "") {
        ...
        if (isNumeric == false) {
          validated = false;
          if (
            item.id == "minTemperatureFilter" ||
            item.id == "maxTemperatureFilter"
          ) {
            $(itemId).after(
              '<label id="' +
                item.id +
                ...
                item.id +
                '">Please enter a vaule between -100 and 200.</label>'
            );
          } else {
            $(itemId).after(
              '<label id="' +
                ...
                '-error" class="error" for="' +
                item.id +
                '">Please enter a vaule between 0 and 300.</label>'
            );
          }
        } else if (
          item.id == "minTemperatureFilter" ||
  ...
  if ($("#rowKeyDateStartFilter").val() != "") {
    data =
      data + '"rowKeyDateStart":"' + $("#rowKeyDateStartFilter").val() + '",';
  } else {
  ...
  if ($("#rowKeyTimeStartFilter").val() != "") {
    data =
      data + '"rowKeyTimeStart":"' + $("#rowKeyTimeStartFilter").val() + '",';
  } else {
  ...
  if ($("#minPrecipitationFilter").val() != "") {
    data =
      data + '"minPrecipitation":' + $("#minPrecipitationFilter").val() + ",";
  } else {
  ...
  if ($("#maxPrecipitationFilter").val() != "") {
    data =
      data + '"maxPrecipitation":' + $("#maxPrecipitationFilter").val() + ",";
  } else {
  ...
  $.ajax({
    ...
    success: function (data, status, xhr) {
      if (xhr.status == 200) {
        ...
        var errorHtml =
          '<div class="alert alert-danger alert-dismissible fade show" role="alert" id="filterErrorArea">' +
          xhr.status +
    ...
    success: function (data, status, xhr) {
      ...
      if (xhr.status == 200) {
        ...
        $.each(data.entitiesList, function (entityIndex, item) {
          ...
          html =
            html +
            '<button id="updateEntityButton_' +
            entityIndex +
            '" class="btn btn-primary btn-sm entity-update" data-bs-toggle="modal" data-bs-target="#updateEntityModel">Update</button>&nbsp;';
          html =
            html +
            '<button id="removeEntityButton_' +
            entityIndex +
        ...
        var errorHtml =
          '<div class="alert alert-danger alert-dismissible fade show" role="alert" id="indexErrorArea">' +
          "statusText: " +
          xhr.statusText +
          '<button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button></div>';
...
function insertTableEntity() {
  ...
  $("input[id$='TableEntity']").each(function (index, item) {
    var itemId = "#" + item.id;
    if (
      item.id == "stationNameTableEntity" ||
  ...
  var data =
    "{" +
    '"partitionKey":"' +
    $("#stationNameTableEntity").val() +
    '",' +
    '"rowKey":"' +
    $("#observationDateTableEntity").val() +
    " " +
    $("#observationTimeTableEntity").val() +
    '",' +
    '"observationTime":"' +
    $("#observationTimeTableEntity").val() +
    '",' +
    '"Temperature":' +
    $("#temperatureTableEntity").val() +
    "," +
    '"Humidity":' +
    $("#humidityTableEntity").val() +
    "," +
    '"Barometer":' +
    $("#barometerTableEntity").val() +
    "," +
    '"WindDirection":"' +
    $("#windDirectionTableEntity").val() +
    '",' +
    '"WindSpeed":' +
    $("#windSpeedTableEntity").val() +
    "," +
    '"Precipitation":' +
    $("#precipitationTableEntity").val() +
    "}";
  $.ajax({
    ...
    success: function (data, status, xhr) {
      ...
      if (xhr.status == 200) {
        ...
        var errorHtml =
          '<div class="alert alert-danger alert-dismissible fade show" role="alert" id="usingTableEntityErrorArea">' +
          xhr.status +
          '<button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button></div>';
...
function insertExpandableData() {
  ...
  var data =
    "{" +
    '"partitionKey":"' +
    $("#stationNameExpandable").val() +
    '",' +
    '"rowKey":"' +
    $("#observationDateExpandable").val() +
    " " +
    $("#observationTimeExpandable").val() +
    '",' +
    '"propertyMap":' +
    "{";
    ...
    success: function (data, status, xhr) {
      if (xhr.status == 200) {
        ...
        var errorHtml =
          ...
          "</div>";
        var errorHtml =
          '<div class="alert alert-danger alert-dismissible fade show" role="alert" id="usingExpandedDataErrorArea">' +
          xhr.responseText +
          '<button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button></div>';
    ...
    success: function (data, status, xhr) {
      if (xhr.status == 200) {
        ...
        var errorHtml =
          ...
          "</div>";
        var errorHtml =
          '<div class="alert alert-danger alert-dismissible fade show" role="alert" id="sampleDataErrorArea">' +
          xhr.responseText +
          '<button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button></div>';
...
function showUpdateEntityModel(entity, listOfKeys) {
  var html =
    ...
    "     </div>" +
    '     <div class="col-sm-8">' +
    '           <input type="text" id="PartitionKeyUpdate" name="stationName" class="form-control" value="' +
    entity.partitionKey +
    '" readonly="readonly">' +
    "     </div>" +
    "</div>" +
    ...
    "   </div>" +
    '   <div class="col-sm-8">' +
    '       <input type="text" id="RowKeyUpdate" name="observationDate" class="form-control" value="' +
    entity.rowKey +
    '" readonly="readonly">' +
    "   </div>" +
    "</div>" +
...
function updateTableEntity() {
  ...
  var data =
    ...
    "{";
  $("input[id$='Update']").each(function (index, item) {
    if (item.id != "PartitionKeyUpdate" && item.id != "RowKeyUpdate") {
      data = data + '"' + item.name + '":"' + item.value + '",';
    }
  });
  ...
  $.ajax({
    ...
    success: function (data, status, xhr) {
      if (xhr.status == 200) {
        ...
        var errorHtml =
          '<div class="alert alert-danger alert-dismissible fade show" role="alert" id="updateEntityErrorArea">' +
          xhr.responseText +
          '<button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button></div>';
...
function addCustomField() {
  ...
  var html =
    '<div id="' +
    convertedFieldValue +
    'CustomExpandableDiv" class="row g-3 mb-2">' +
    '<div class="col-sm-4">' +
    '<label for="' +
    convertedFieldValue +
    'Expandable" class="col-form-label">' +
    fieldValue +
    "</label>" +
    "</div>" +
    '<div class="col-sm-7">' +
    ...
    '   <button id="' +
    convertedFieldValue +
    'ExpandableRemove" class="btn btn-danger">X</button>';
  "</div>" + "</div>";
